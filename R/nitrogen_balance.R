...
n_balance <- function(para, land_required, energy_required = NULL, soil_erosion = NULL) {
  ...
  for (feed in feed_types) {
    ...
    feed_production <- tidyr::unnest(para[["feed_items"]], cols = c(crop_name))
    ...
    n_fixing <- ifelse(
      grepl("legume", feed_selected$category, ignore.case = TRUE),
      0.5 * (residue_n * residue_dry_yield + main_n * dry_yield) * 1000,
      0
    )

    feed_selected_land_required <-
      land_required[["land_requirements_all"]][land_required[["land_requirements_all"]]$feed == feed, ]
    ...
    if (length(para[["fertilizer"]]) == 0) {
      ...
      ammonia_n_frac <- ifelse(
        feed_selected$ammonia == 0, 0,
      ...
      ammonium_nitrate_n_frac <- ifelse(
        feed_selected$ammonium_nitrate == 0, 0,
      ...
      ammonium_sulfate_n_frac <- ifelse(
        feed_selected$ammonium_sulfate == 0, 0,
      ...
      n_solutions_n_frac <- ifelse(
        feed_selected$n_solutions == 0, 0,
    ...
    fertilizer_rate <-
      (feed_selected$ammonia * ammonia_n_frac) +
      (feed_selected$ammonium_nitrate * ammonium_nitrate_n_frac) +
      (feed_selected$ammonium_sulfate * ammonium_sulfate_n_frac) +
      (feed_selected$dap * dap_n_frac) +
      (feed_selected$n_solutions * n_solutions_n_frac) +
      (feed_selected$npk * npk_n_frac) +
      (feed_selected$urea * urea_n_frac)
    ...
    sum_n_content_manure_grazing <- energy_required[["annual_results"]] %>%
      as.data.frame() %>%
      dplyr::summarise(val = sum(n_content_manure_grazing, na.rm = TRUE)) %>%
      dplyr::pull(val)
    ...
    n_content_manure_collected <- energy_required[["annual_results"]] %>%
      as.data.frame() %>%
      dplyr::summarise(val = sum(n_content_manure_collected, na.rm = TRUE)) %>%
      dplyr::pull(val)

    animal_manure_collected <- n_content_manure_collected * manure_fraction

    organic_n_imported <- manure_fraction * (
      as.numeric(para$purchased_manure) +
      as.numeric(para$purchased_compost) +
      as.numeric(para$purchased_organic_n) +
      as.numeric(para$purchased_bedding)
    )
    ...
    soil_loss_plot <- sum(
      as.numeric(soil_erosion[soil_erosion$feed_type == feed, ]$soil_loss_plot),
      na.rm = TRUE
    )
    ...
    n_balance[[feed]] <- data.frame(
      feed = feed,
      n_fixing = n_fixing,
      area_total = area_total,
      fertilizer_rate = fertilizer_rate,
      sum_n_content_manure_grazing = sum_n_content_manure_grazing,
      animal_manure_collected = animal_manure_collected,
      organic_n_imported = organic_n_imported,
      yield_dm_ha = yield_dm_ha,
      crop_residue_dm_ha = crop_residue_dm_ha,
      residue_removal = residue_removal,
      main_product_removal = main_product_removal,
      main_product_removed_kg_ha = main_product_removed_kg_ha,
      main_product_removed_kg = main_product_removed_kg,
      residue_removed_dm_ha = residue_removed_dm_ha,
      residue_removed_kg = residue_removed_kg,
      annual_precipitation = annual_precipitation,
      soil_n = soil_n,
      ntot_kg_ha_20cm = ntot_kg_ha_20cm,
      n_mineralized_kg_ha_year = n_mineralized_kg_ha_year,
      soil_type = soil_type,
      soil_c = soil_c,
      soil_clay = soil_clay,
      ncrop = ncrop,
      ...
      out2 = out2,
      out5 = out5,
      nfertilizer = nfertilizer,
      stringsAsFactors = FALSE
    )
  }

  n_balance_all <- dplyr::bind_rows(n_balance)

  n_balance_all <- n_balance_all %>%
    dplyr::mutate(dplyr::across(-c(feed, soil_type), as.numeric))

  denom <- sum(n_balance_all$main_product_removed_kg, na.rm = TRUE) +
    sum(n_balance_all$residue_removed_kg, na.rm = TRUE)

  n_balance_all <- n_balance_all %>%
    dplyr::mutate(
      animal_manure_grazing = ifelse(
        denom > 0,
        sum_n_content_manure_grazing * (main_product_removed_kg + residue_removed_kg) / denom,
        0
      ),
      organic_n_kg_total = animal_manure_grazing + animal_manure_collected + organic_n_imported,
      organic_n_kg_per_ha = ifelse(area_total > 0, organic_n_kg_total / area_total, 0)
    ) %>%
    dplyr::mutate(dplyr::across(where(is.numeric), ~ replace(., !is.finite(.), 0))) %>%
    tidyr::replace_na(as.list(rep(0, ncol(.) - 2)))
  ...
  n_balance_all <- n_balance_all %>%
    dplyr::mutate(
      ...
      nue = ifelse(nin > 0, nout / nin, 0),
      nbalance_kg_n_total = nin - nout,
      nbalance_kg_n_ha_total = ifelse(area_total > 0, nbalance_kg_n_total / area_total, 0),
      nbalance_feed_only_kg_n = ifelse(
        nbalance_kg_n_total == 0, 0,
        ifelse(out2 == 0, nbalance_kg_n_total, nbalance_kg_n_total * out2 / (out2 + out1))
      ),
      nbalance_feed_only_kg_n_ha = ifelse(area_total > 0, nbalance_feed_only_kg_n / area_total, 0)
    ) %>%
    dplyr::mutate(dplyr::across(where(is.numeric), ~ replace(., !is.finite(.), 0))) %>%
    tidyr::replace_na(as.list(rep(0, ncol(.) - 2))) %>%
    dplyr::mutate(
      rough_of_kg_n = ifelse(stringr::str_detect(feed, "OFR"), nbalance_feed_only_kg_n, 0),
