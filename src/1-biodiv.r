...
# read libraries
library(pacman)
pacman::p_load(raster, maptools, RColorBrewer)

# read parameter
tspecies <- read.csv(paste0(path, "/1-input/parameter/threatenedspecies.csv"))
tspecies[is.na(tspecies)] <- 0

# run the landuse change scenario if relevant

bio_base <- read.csv(paste0(path, "/1-input/base_run/bio_ind.csv"), row.names = NULL)
...
fun <- function(a, b, c, d, e, f, g, h, i, j, k, l, m) {
  a + b + c + d + e + f + g + h + i + j + k + l + m
}
bio_index <- overlay(bi_acrop, bi_cbush, bi_cgrass, bi_dforest, bi_maforest, 
                     bi_mforest, bi_obush, bi_ograss, bi_pcrop, bi_pforest, bi_sforest, 
                     bi_waterbody, bi_wetland, fun = fun)
...
if (add > 0) {
  ...
  # computing habitat change for endangered species due to scenario
  # conversion to annual cropland note it is overwriting variables from
  ...
  fun <- function(b, c, d, e, f, g, h, i, j, k, l, m) {
    b + c + d + e + f + g + h + i + j + k + l + m
  }
  esp_sc <- overlay(bis_cbush, bis_cgrass, bis_dforest, bis_maforest, 
                    bis_mforest, bis_obush, bis_ograss, bis_pcrop, bis_pforest, bis_sforest, 
                    bis_waterbody, bis_wetland, fun = fun)
  
  # esp_sc<-zoom(esp_sc, ext=addcrop@extent) plot(esp_sc)
  
}


################## landscape level indicators ######### average biodiversity index
...
bio_ind2$evaluation <- ifelse(bio_ind2$percent <= bio_ind_val$val, "low", 
                              ifelse((bio_ind2$percent >= bio_ind_val$val) & (bio_ind2$percent <= 
                                                                                2 * bio_ind_val$val), "medium", "high"))
...
title2 <- paste("Biodiversity pathway : critically affected species", name, 
                sep = " ")
title3 <- paste("Biodiversity pathway : difference of richness index", 
                name, sep = " ")
...
# plot(bio_index, legend.width=1, legend.shrink=0.75,col=col)
# plot(sarea, add=TRUE) title(title1) dev.off()
